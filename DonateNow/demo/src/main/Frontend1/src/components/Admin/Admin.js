...
add()
  ...
  const deleteCharity = (id)=>{
    axios
    .delete(`http://localhost:8081/charities/${id}`,config)
    .then(()=>{
  ...
  return (
      <>
            ...
            <Toolbar>
                <NavLink className={classe.tabs} to="/"
                   
                      onClick={() => {
                        const action = logout();
                        dispatch(action);
                      }}
                   > <ExitToAppIcon className="icon-logout-admin"/></NavLink>
                <NavLink className={classe.tabs} to="add" exact>Add Charity</NavLink>
            </Toolbar>
        </AppBar>
      ...
      <TableHead>
        ...
        <TableRow>
          ...
          <Button
            ...
            <Button color="secondary" variant="contained"onClick={()=>{deleteCharity(e.id);}}>
              <DeleteIcon/> 
            </Button>
          </TableCell></TableRow>
      </TableBody>
